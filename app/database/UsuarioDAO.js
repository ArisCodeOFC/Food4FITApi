...
class UsuarioDAO {
    ...
    checarEmailExistente(email, callback) {
        this.connection.query("SELECT 1 FROM tbl_usuario WHERE email = ?", [email], callback);
    }

    trocarSenha(email, senha, callback) {
        const hash = crypto.createHash("md5").update(senha).digest("hex");
        this.connection.query("UPDATE tbl_usuario SET senha = ? WHERE email = ?", [hash, email], callback);
    }
}
